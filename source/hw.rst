...
- Each Control Unit has a program counter (PC) and controls tens of processing
  units (PUs).

- Each processing unit (PU) has its own general-purpose register set (GPR) and
  stack memory.
...
.. note:: A SIMD thread executed by a SIMD processor (also known as an SM) has
   16 lanes.
...
- Two levels of scheduling:
  ...
  - Level 1: Thread Block Scheduler
    ...
    When an SM executes a Thread Block, the threads within the block execute
    together. If any thread in a Warp is not ready because of
...
**Comparison of Volta and Pascal**
...
3. Texture Filtering
   ...
   * Common filtering modes as the following are shown as 
     ...
     - Let's see how these methods work when using a texture with a low 
       ...
       As a result, GL_LINEAR produces a blurrier image with smoother edges.
